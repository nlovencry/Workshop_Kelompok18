<?php
	...
	class Library{
	    ...
	    {
	        $data = $this->db->prepare('INSERT INTO tb_mhs (nim,nama_mhs,prodi,alamat) VALUES (?, ?, ?, ?)');
	    ...
	    {
	        $query = $this->db->prepare("SELECT * FROM tb_mhs");
	        $query->execute();
	        $data = $query->fetchAll();
	        return $data;
	    }
	    ...
	    public function get_by_id($nim){
	        $query = $this->db->prepare("SELECT * FROM tb_mhs where nim=?");
	    ...
	    public function update($nim,$nama_mhs,$prodi,$alamat){
	        $query = $this->db->prepare('UPDATE tb_mhs set nama_mhs=?,prodi=?,alamat=? where nim=?');
	    ...
	    {
	        $query = $this->db->prepare("DELETE FROM tb_mhs where nim=?");
